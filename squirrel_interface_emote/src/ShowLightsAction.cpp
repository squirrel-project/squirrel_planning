...
namespace KCL_rosplan {
	...
	void ShowLightsAction::dispatchCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg)
	{
		...
		if (normalised_action_name != "show_lights" || msg->parameters.size() != 3)
		{
			return;
		}
		...
		const std::string& emotion = msg->parameters[2].value;
		
		ROS_INFO("KCL: (ShowLightsAction) Process the action: %s, %s, %s: show lights to %s", normalised_action_name.c_str(), robot.c_str(), child.c_str(), emotion.c_str());
		...
		std::size_t red_pos = emotion.find('_');
		std::size_t green_pos = emotion.find('_', red_pos);
		
		float red = ::atof(emotion.substr(0, red_pos).c_str());
		float green = ::atof(emotion.substr(red_pos + 1, green_pos).c_str());
		float blue = ::atof(emotion.substr(green_pos + 1).c_str());
