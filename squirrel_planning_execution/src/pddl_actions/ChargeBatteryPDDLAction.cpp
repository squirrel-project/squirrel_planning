...
#include "ChargeBatteryPDDLAction.h"

namespace KCL_rosplan
{

ChargeBatteryPDDLAction::ChargeBatteryPDDLAction(ros::NodeHandle& node_handle)
{
	// knowledge interface
	...
	dispatch_sub_ = node_handle.subscribe("/kcl_rosplan/action_dispatch", 1000, &KCL_rosplan::ChargeBatteryPDDLAction::dispatchCallback, this);
}

ChargeBatteryPDDLAction::~ChargeBatteryPDDLAction()
{
	
}

void ChargeBatteryPDDLAction::dispatchCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg)
{
	...
	if (normalised_action_name != "charge_battery")
	{
		return;
	}
	
	ROS_INFO("KCL: (ChargeBatteryPDDLAction) Process the action: %s", normalised_action_name.c_str());
