...
#include "ClassifyObjectInHandPDDLAction.h"

namespace KCL_rosplan
{

ClassifyObjectInHandPDDLAction::ClassifyObjectInHandPDDLAction(ros::NodeHandle& node_handle, float classification_probability)
	: classification_probability_(classification_probability), ask_user_input_(false)
{
	...
	dispatch_sub_ = node_handle.subscribe("/kcl_rosplan/action_dispatch", 1000, &KCL_rosplan::ClassifyObjectInHandPDDLAction::dispatchCallback, this);
...
ClassifyObjectInHandPDDLAction::~ClassifyObjectInHandPDDLAction()
{
	
}

void ClassifyObjectInHandPDDLAction::dispatchCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg)
{
	...
	if (normalised_action_name != "classify_object_in_hand" || msg->parameters.size() != 2)
	{
		return;
	}
	...
	ROS_INFO("KCL: (ClassifyObjectInHandPDDLAction) Process the action: (%s %s %s)", normalised_action_name.c_str(), robot.c_str(), object.c_str());
	...
	else 
	{
		...
		ROS_INFO("KCL: (ClassifyObjectInHandPDDLAction) Is negative? %f >= %f.", p, classification_probability_);
	...
	if (!update_knowledge_client_.call(knowledge_update_service)) {
		ROS_ERROR("KCL: (ClassifyObjectInHandPDDLAction) Could not add the classifiable_from predicate to the knowledge base.");
		exit(-1);
	}
	ROS_INFO("KCL: (ClassifyObjectInHandPDDLAction) Added %s (is_examined %s) to the knowledge base.", knowledge_item.is_negative ? "NOT" : "", object.c_str());
	...
	knowledge_update_service.request.knowledge = knowledge_item;
	if (!update_knowledge_client_.call(knowledge_update_service)) {
		ROS_ERROR("KCL: (ClassifyObjectInHandPDDLAction) Could not remove the classifiable_from predicate to the knowledge base.");
// 		exit(-1);
	}
	ROS_INFO("KCL: (ClassifyObjectInHandPDDLAction) Removed %s (is_examined %s) to the knowledge base.", knowledge_item.is_negative ? "NOT" : "", object.c_str());
	...
	if (classification_succeeded)
	{
		...
		{
			ROS_ERROR("KCL: (ClassifyObjectInHandPDDLAction) Could not get the instances of type 'type'.");
			exit(1);
		}
		if (get_instance.response.instances.empty())
		{
			ROS_ERROR("KCL: (ClassifyObjectInHandPDDLAction) Got instances of type 'type', but there are none!");
			exit(1);
		}
		...
		if (!update_knowledge_client_.call(knowledge_update_service))
		{
			ROS_ERROR("KCL: (ClassifyObjectInHandPDDLAction) Could not add the (is_of_type %s %s) predicate to the knowledge base.", object.c_str(), type.c_str());
			exit(1);
		}
		ROS_INFO("KCL: (ClassifyObjectInHandPDDLAction) Added the (is_of_type %s %s) predicate to the knowledge base.", object.c_str(), type.c_str());
		...
		if (!update_knowledge_client_.call(knowledge_update_service))
		{
			ROS_ERROR("KCL: (ClassifyObjectInHandPDDLAction) Could not remove the (not (is_of_type %s %s)) predicate to the knowledge base.", object.c_str(), type.c_str());
			exit(1);
		}
		ROS_INFO("KCL: (ClassifyObjectInHandPDDLAction) Removed the (not (is_of_type %s %s)) predicate to the knowledge base.", object.c_str(), type.c_str());
		...
		for (std::vector<std::string>::const_iterator ci = get_instance.response.instances.begin(); ci != get_instance.response.instances.end(); ++ci)
		{
			...
			if (type != other_type)
			{
				...
				if (!update_knowledge_client_.call(knowledge_update_service))
				{
					ROS_ERROR("KCL: (ClassifyObjectInHandPDDLAction) Could not add the (not (is_of_type %s %s)) predicate to the knowledge base.", object.c_str(), other_type.c_str());
					exit(1);
				}
				ROS_INFO("KCL: (ClassifyObjectInHandPDDLAction) Added the (not (is_of_type %s %s)) predicate to the knowledge base.", object.c_str(), other_type.c_str());
				...
				if (!update_knowledge_client_.call(knowledge_update_service))
				{
					ROS_ERROR("KCL: (ClassifyObjectInHandPDDLAction) Could not remove the (is_of_type %s %s) predicate to the knowledge base.", object.c_str(), other_type.c_str());
					exit(1);
				}
				ROS_INFO("KCL: (ClassifyObjectInHandPDDLAction) Removed the (is_of_type %s %s) predicate to the knowledge base.", object.c_str(), other_type.c_str());
			}
		}
	}
