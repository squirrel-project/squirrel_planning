...
#include "ChildPickupPDDLAction.h"
#include "squirrel_planning_execution/ViewConeGenerator.h"

namespace KCL_rosplan
{

ChildPickupPDDLAction::ChildPickupPDDLAction(ros::NodeHandle& node_handle)
{
	// knowledge interface
	...
	dispatch_sub_ = node_handle.subscribe("/kcl_rosplan/action_dispatch", 1000, &KCL_rosplan::ChildPickupPDDLAction::dispatchCallback, this);
}

ChildPickupPDDLAction::~ChildPickupPDDLAction()
{
	
}

void ChildPickupPDDLAction::dispatchCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg)
{
	...
	if (normalised_action_name != "child-pickup")// || msg->parameters.size() != 3)
	{
		return;
	}
	
	ROS_INFO("KCL: (ChildPickupPDDLAction) Process the action: %s", normalised_action_name.c_str());
	/*
	for (std::vector<diagnostic_msgs::KeyValue>::const_iterator ci = msg->parameters.begin(); ci != msg->parameters.end(); ++ci)
	{
		...
		ROS_INFO("KCL: (ChildPickupPDDLAction) %s -> %s", key.c_str(), value.c_str());
	}
	*/
	...
	const std::string& object_waypoint = msg->parameters[2].value;
	const std::string& child = msg->parameters[3].value;
	
	
	//const std::string& type = msg->parameters[4].value;
	
	ROS_INFO("KCL: (ChildPickupPDDLAction) Process the action: %s, %s given by %s to %s", normalised_action_name.c_str(), object.c_str(), child.c_str(), robot.c_str());
	...
	if (!update_knowledge_client_.call(knowledge_update_service)) {
		ROS_ERROR("KCL: (ChildPickupPDDLAction) Could not remove the previous (object_at %s %s) predicate from the knowledge base.", object.c_str(), object_waypoint.c_str());
		exit(-1);
	}
	ROS_INFO("KCL: (ChildPickupPDDLAction) Removed the previous (object_at %s %s) predicate from the knowledge base.", object.c_str(), object_waypoint.c_str());
	...
	knowledge_update_service.request.knowledge = kenny_knowledge;
	if (!update_knowledge_client_.call(knowledge_update_service)) {
		ROS_ERROR("KCL: (ChildPickupPDDLAction) Could not add the new (child_holding %s %s) predicate to the knowledge base.", robot.c_str(), object.c_str());
		exit(-1);
	}
	ROS_INFO("KCL: (ChildPickupPDDLAction) Added the new (child_holding %s %s) predicate to the knowledge base.", robot.c_str(), object.c_str());
