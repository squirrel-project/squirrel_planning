...
{
	ROS_INFO("KCL: (OxygenBattery) Load scenarion from file: %s.\n", config_file.c_str());
	...
	if (f.is_open())
	{
		while (getline(f, line))
		{
			...
			if (line[0] == 'b')
			{
				if (tokens.size() != 4)
				{
					ROS_ERROR("KCL (OxygenBattery) Malformed line, expected b BOX_NAME (f,f,f) (f,f,f). Read %s\n", line.c_str());
					exit(0);
				}
				...
				if (!update_knowledge_client.call(knowledge_update_service)) {
					ROS_ERROR("KCL: (OxygenBattery) Could not add the box %s to the knowledge base.", box_predicate.c_str());
					exit(-1);
				}
				ROS_INFO("KCL: (OxygenBattery) Added %s to the knowledge base.", box_predicate.c_str());
				...
				if (!update_knowledge_client.call(knowledge_update_service)) {
					ROS_ERROR("KCL: (OxygenBattery) Could not add the waypoint %s to the knowledge base.", ss.str().c_str());
					exit(-1);
				}
				ROS_INFO("KCL: (OxygenBattery) Added %s to the knowledge base.", ss.str().c_str());
				...
				knowledge_update_service.request.knowledge = knowledge_item;
				if (!update_knowledge_client.call(knowledge_update_service)) {
					ROS_ERROR("KCL: (OxygenBattery) Could not add the fact (box_at %s %s) to the knowledge base.", box_predicate.c_str(), ss.str().c_str());
					exit(-1);
				}
				ROS_INFO("KCL: (OxygenBattery) Added the fact (box_at %s %s) to the knowledge base.", box_predicate.c_str(), ss.str().c_str());
			...
			else if (line[0] == 'w')
			{
				...
				if (tokens.size() != 3)
				{
					ROS_ERROR("KCL (OxygenBattery) Malformed line, expected w WAYPOINT_NAME (f,f,f). Read %s\n", line.c_str());
					exit(0);
				}
				...
				knowledge_update_service.request.knowledge = knowledge_item;
				if (!update_knowledge_client.call(knowledge_update_service)) {
					ROS_ERROR("KCL: (OxygenBattery) Could not add the waypoint %s to the knowledge base.", waypoint_predicate.c_str());
					exit(-1);
				}
				ROS_INFO("KCL: (OxygenBattery) Added %s to the knowledge base.", waypoint_predicate.c_str());
			}
			else if (line[0] == 't')
			{
				if (tokens.size() != 3)
				{
					ROS_ERROR("KCL (OxygenBattery) Malformed line, expected t TOY_NAME (f,f,f). Read %s\n", line.c_str());
					exit(0);
				}
				...
				ROS_INFO("KCL: (OxygenBattery) Added %s to the knowledge base.", toy_predicate.c_str());
			}
			else if (line[0] == 'm')
			{
				if (tokens.size() != 3)
				{
					ROS_ERROR("KCL (OxygenBattery) Malformed line, expected m OBJECT_ID BOX_NAME. Read %s\n", line.c_str());
					exit(0);
				}
				...
				knowledge_update_service.request.knowledge = knowledge_item;
				if (!update_knowledge_client.call(knowledge_update_service)) {
					ROS_ERROR("KCL: (OxygenBattery) Could not add the fact (belongs_in %s %s) to the knowledge base.", object_id.c_str(), box_name.c_str());
					exit(-1);
				}
				ROS_ERROR("KCL: (OxygenBattery) Added the fact (belongs_in %s %s) to the knowledge base.", object_id.c_str(), box_name.c_str());
			}
		}
	}
	...
	// Set kenny at it's starting waypoint.
	{
		...
		knowledge_update_service.request.knowledge = knowledge_item;
		if (!update_knowledge_client.call(knowledge_update_service)) {
			ROS_ERROR("KCL: (OxygenBattery) Could not add the robot robot to the knowledge base.");
			exit(-1);
		}
		ROS_INFO("KCL: (OxygenBattery) Added robot to the knowledge base.");
	}
	...
	knowledge_update_service.request.knowledge = knowledge_item;
	if (!update_knowledge_client.call(knowledge_update_service)) {
		ROS_ERROR("KCL: (OxygenBattery) Could not add the fact (robot_at robot kenny_waypoint) to the knowledge base.");
		exit(-1);
	}
...
int main(int argc, char **argv) {
	...
	std::stringstream ss;
	//ss << data_path << "tidy_room_domain-nt.pddl";
	ss << data_path << "oxygen_battery_domain.pddl";
	std::string domain_path = ss.str();
	...
	std::string planner_command;
	ss.str(std::string());
	ss << "cat " << data_path << "/oxygen_battery_plan.pddl";
	...
	ROS_INFO("KCL: (OxygenBattery) Start plan action");
	...
	ROS_INFO("KCL: (OxygenBattery) Start planning server found");
	
	// send goal
	plan_action_client.sendGoal(psrv);
	ROS_INFO("KCL: (OxygenBattery) Goal sent");
